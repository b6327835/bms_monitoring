...
export default function EquipmentOverviewPanels({
  ...
  return (
    <>
      ...
      {chillerPanelOpen && (
        <DraggablePanel panelId="chillerPanel" title="Chiller System" position={chillerPanelPos} setPosition={setChillerPanelPos} minimized={chillerPanelMin} setMinimized={setChillerPanelMin} onClose={closeChillerPanel} width={600} onDragStart={onDragStart} onDragEnd={onDragEnd} zIndex={getPanelZIndex('chillerPanel')} onPanelClick={bringPanelToFront}>
          <div>
            {chillers.map((chiller) => {
              ...
              return (
                <div key={chiller.id} style={{ border: '1px solid #1f2937', borderRadius: '10px', padding: '10px', marginBottom: '10px', background: '#0b1220' }}>
      ...
      {/* AHU panel (draggable) */}
      {ahuPanelOpen && (
        <DraggablePanel panelId="ahuPanel" title="AHU System" position={ahuPanelPos} setPosition={setAhuPanelPos} minimized={ahuPanelMin} setMinimized={setAhuPanelMin} onClose={closeAhuPanel} width={600} onDragStart={onDragStart} onDragEnd={onDragEnd} zIndex={getPanelZIndex('ahuPanel')} onPanelClick={bringPanelToFront}>
          <div>
            {ahus.map((ahu) => {
              const statusColor = ahu.status === 'fault' ? '#ef4444' : '#10b981';
              const statusText = ahu.status.charAt(0).toUpperCase() + ahu.status.slice(1);
              return (
                <div key={ahu.id} style={{ border: '1px solid #1f2937', borderRadius: '10px', padding: '10px', marginBottom: '10px', background: '#0b1220' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
      ...
      {electricalPanelOpen && (
        <DraggablePanel panelId="electricalPanel" title="Electrical Panel" position={electricalPanelPos} setPosition={setElectricalPanelPos} minimized={electricalPanelMin} setMinimized={setElectricalPanelMin} onClose={closeElectricalPanel} width={600} onDragStart={onDragStart} onDragEnd={onDragEnd} zIndex={getPanelZIndex('electricalPanel')} onPanelClick={bringPanelToFront}>
          <div>
            {electricals.map((electrical) => {
              ...
              return (
                <div key={electrical.id} style={{ border: '1px solid #1f2937', borderRadius: '10px', padding: '10px', marginBottom: '10px', background: '#0b1220' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
      ...
      {pumpPanelOpen && (
        <DraggablePanel panelId="pumpPanel" title="Water Pump System" position={pumpPanelPos} setPosition={setPumpPanelPos} minimized={pumpPanelMin} setMinimized={setPumpPanelMin} onClose={closePumpPanel} width={600} onDragStart={onDragStart} onDragEnd={onDragEnd} zIndex={getPanelZIndex('pumpPanel')} onPanelClick={bringPanelToFront}>
          <div>
            {pumps.map((pump) => {
              const statusColor = pump.status === 'fault' ? '#ef4444' : '#10b981';
              const statusText = pump.status.charAt(0).toUpperCase() + pump.status.slice(1);
              return (
                <div key={pump.id} style={{ border: '1px solid #1f2937', borderRadius: '10px', padding: '10px', marginBottom: '10px', background: '#0b1220' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
      ...
      {firePanelOpen && (
        <DraggablePanel panelId="firePanel" title="Fire Alarm System" position={firePanelPos} setPosition={setFirePanelPos} minimized={firePanelMin} setMinimized={setFirePanelMin} onClose={closeFirePanel} width={600} onDragStart={onDragStart} onDragEnd={onDragEnd} zIndex={getPanelZIndex('firePanel')} onPanelClick={bringPanelToFront}>
          <div>
            {fires.map((fire) => {
              const statusColor = fire.status === 'fault' ? '#ef4444' : '#10b981';
              const statusText = fire.status.charAt(0).toUpperCase() + fire.status.slice(1);
              return (
                <div key={fire.id} style={{ border: '1px solid #1f2937', borderRadius: '10px', padding: '10px', marginBottom: '10px', background: '#0b1220' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
